...
class HarDNetBase(nn.Module):
    def __init__(self, arch, depth_wise=False, Maxpool=False):
        super().__init__()
        if arch == 85:
          first_ch  = [48, 96]
          second_kernel = 3
...
class HarDNetSeg(nn.Module):
    def __init__(self, num_layers, heads, pretrained, down_ratio, final_kernel,
        ...
        if Maxpool:
            self.last_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        else:
        ...
        for i in range(3):
            ...
            self.transUpBlocks.append(TransitionUp(channels[3-i], channels[2-i]))
            self.conv1x1_up.append(ConvLayer(channels[3-i]+channels[2-i], channels[2-i], kernel=1))

        prev_ch = channels[0]
        weights_init(self.transUpBlocks)
